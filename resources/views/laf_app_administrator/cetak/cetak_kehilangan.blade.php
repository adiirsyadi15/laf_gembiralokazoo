...
<body>
	...
	<div id="konten">
		...
		@endif
		<br>
		<label>Yang bernama</label><span class="tab">: {{ $k->nama }}</span><br>
		<label>Alamat</label><span class="tab">: {{ $k->alamat }}</span><br>
		<label>Umur</label><span class="tab">: {{ $umur }} tahun</span><br>
		<label>Tempat/Tanggal Lahir</label><span class="tab">: {{ $k->tempat_lahir }} , {{$k->tanggal_lahir}}</span><br>
		<label>Pekerjaan</label><span class="tab">: {{ $k->pekerjaan }}</span><br>
		<label>Nomor hp </label><span class="tab">: {{ $k->no_hp }}</span><br>
		...
		<div id="tanda_tangan">
			...
			<div id="penandatangan">
				@foreach($kehilangans as $k)
				<p>( {{ $k->nama }} )<span class="tab_petugas">(  {{ $k->nama_petugas }} )</span></p>
				@endforeach
			</div>
		</div>
